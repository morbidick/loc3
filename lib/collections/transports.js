...
Meteor.methods({
  addTransport: function(transport) {
    validate.authorized(Meteor.user(), "transport-add");
    ...
    if (!validate.isText(transport.shipper)) {
      throw new Meteor.Error("invalid-input", "Shipper needs to be text.");
    }
    ...
    transport = validate.stripTo(transport, ["name", "prioritised", "arrival", "departure", "vendor", "home", "shipper", "is_ccc", "comment", "files"])
